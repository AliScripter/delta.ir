<template>
  <b-card no-body class="border overflow-hidden p-0 h-100">
    ...
    <b-card-footer class="bg-transparent d-flex justify-content-between align-items-center pt-0">
      <div class="d-flex align-items-center gap-2 justify-content-center">
        <template v-if="product.sale">
          <p v-if="lang==='en'" class="fw-bold text-success mb-0">
            {{ currency }}{{ discountedPrice(product.price, product.sale.discount, product.sale.type) }}
          </p>
          <p v-else-if="lang==='fa'" class="fw-bold text-success mb-0">
            {{ discountedPrice(product.price, product.sale.discount, product.sale.type) }} {{ currency }}
          </p>
          <small v-if="lang==='en'" class="text-decoration-line-through">{{ currency }}{{ product.price }}</small>
          <small v-else-if="lang==='fa'" class="text-decoration-line-through">{{ product.price }} {{ currency }}</small>
        </template>
      ...
      <a href="#" class="btn btn-dark mb-0 z-index-2">خرید کنید</a>
    </b-card-footer>
  </b-card>
</template>
