    ...
    <b-container>
      <b-row class="g-4 align-items-center">
        <b-col lg="6" class="position-relative">
          <figure class="position-absolute top-0 start-0 mt-n6 ms-n5 z-index-1 rtl:-scale-x-100">
            <svg
              width="117"
              height="98"
              viewBox="0 0 117 98"
          ...
          <img :src="about14" class="rounded" alt="about-img" />
        </b-col>
        <b-col lg="6" class="ps-md-5">
          <h2 class="mb-4">کارایی کسب‌وکار خود را با میزل افزایش دهید.</h2>
          <p class="mb-0">
            حضور آنلاین خود را ارتقا دهید و با تم آژانس دیجیتال طراحی‌شده با دقت ما، سفری به سوی برتری دیجیتال آغاز کنید.
          </p>

          <hr class="my-4" />
          <p>
            در قلب تم ما، تعهد به ارائه یک پلتفرم قدرتمند و چندمنظوره است که نیازهای متنوع آژانس‌های دیجیتال را برآورده می‌کند.
          </p>

          <b-row class="row-cols-1 row-cols-sm-2">
            <b-col v-for="(list, idx) in lists" :key="idx">
              <ul class="list-group list-group-borderless mb-0">
                <li v-for="item in list" :key="item" class="list-group-item d-flex icons-center">
                  <BIconPatchCheckFill class="text-primary me-2" />{{ item }}
                </li>
              </ul>
            </b-col>
          </b-row>
          ...
          <div class="text-center d-inline-block bg-dark rounded px-5 py-3 mt-4 mt-lg-5">
            <span class="text-white">علاقه‌مند به همکاری هستید؟ </span>
            <router-link :to="{ name: 'contact.v2' }" class="ms-2"
              >با ما هم اکنون تماس بگیرید<BIconChevronRight class="small ms-1 rtl:-scale-x-100"
            /></router-link>
          </div>
        </b-col>
      </b-row>

      <b-row class="g-4 mt-5">
        <b-col sm="6" lg="3" v-for="(item, idx) in items" :key="idx">
...
import { items, lists } from '@/views/pages/about/AboutV1/data'
