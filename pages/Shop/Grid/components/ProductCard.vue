  ...
  <b-card no-body class="border bg-transparent overflow-hidden p-0 h-100">
    ...
    >
      <span class="badge text-bg-danger"
        >{{ product.sale.discount }}<span v-if="product.sale.type == 'percent'">%</span> تخفیف
      </span>
    </div>
    <div class="position-absolute start-0 top-0 d-flex w-100 z-index-2 p-3">
      <button
        @click="useShopping.toggleToWishlist(product)"
        class="btn btn-white btn-round border mb-0 ms-auto d-flex align-items-center justify-content-center"
      >
    ...
    <b-card-footer class="bg-transparent d-flex justify-content-between align-items-center pt-0">
      <div class="d-flex align-items-center gap-2 justify-content-center">
        <template v-if="product.sale && product.price">
          <p v-if="lang==='en'" class="fw-bold text-success mb-0">
            {{ currency }}
            {{ discountedPrice(product.price, product.sale.discount, product.sale.type)?.toFixed(2) }}
          </p>
          <p v-else-if="lang==='fa'" class="fw-bold text-success mb-0">
            {{ discountedPrice(product.price, product.sale.discount, product.sale.type)?.toFixed(2) }}
            {{ currency }}
          </p>
          <small v-if="lang==='en'" class="text-decoration-line-through">{{ currency }}{{ product.price.toFixed(2) }}</small>
          <small v-else-if="lang==='fa'" class="text-decoration-line-through">{{ product.price.toFixed(2) }} {{ currency }}</small>
        </template>
        <p v-else class="fw-bold text-success mb-0">
          <span v-if="lang==='en'">{{ currency }}{{ product.price && product.price.toFixed(2) }}</span>
          <span v-else-if="lang==='fa'">{{ product.price && product.price.toFixed(2) }} {{ currency }}</span>
        </p>
      </div>
      <b-button
        variant="dark"
        class="mb-0 z-index-2"
        @click="useShopping.updateQuantity(product, 1)"
      >
        <BIconCart />
      </b-button>
    </b-card-footer>
