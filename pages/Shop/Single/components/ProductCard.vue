  ...
  <b-card no-body class="border bg-transparent overflow-hidden p-0 h-100">
    ...
    <div v-if="product.sale" class="position-absolute top-0 start-0 p-3">
      <span class="badge text-bg-danger"
        >{{ product.sale.discount }}<span v-if="product.sale.type == 'percent'">%</span> تخفیف</span
      >
    </div>
    ...
    <b-card-footer class="bg-transparent d-flex justify-content-between align-items-center pt-0">
      <div class="d-flex align-items-center gap-2 justify-content-center">
        <template v-if="product.sale">
          <p v-if="lang==='en'" class="fw-bold text-success mb-0">{{ currency }}{{ discountedPrice(product.price, product.sale.discount, product.sale.type) }}</p>
          <p v-if="lang==='fa'" class="fw-bold text-success mb-0">{{ discountedPrice(product.price, product.sale.discount, product.sale.type) }} {{ currency }}</p>
          <small v-if="lang==='en'" class="text-decoration-line-through">{{ currency }}{{ product.price }}</small>
          <small v-if="lang==='fa'" class="text-decoration-line-through">{{ product.price }} {{ currency }}</small>
        </template>
        <p v-else class="fw-bold text-success mb-0">
          <span v-if="lang==='en'">{{ currency }}{{ product.price }}</span>
          <span v-if="lang==='fa'">{{ product.price }} {{ currency }}</span>
        </p>
      </div>
