      ...
      <b-row class="g-3 g-sm-4">
        <b-col xl="5">
          ...
          <ul class="nav nav-divider small align-items-center mt-1" v-if="item.product.attributes">
            <li class="nav-item" v-if="item.product.attributes.gender">
              جنسیت: {{ item.product.attributes.gender }}
            </li>
            <li class="nav-item" v-if="item.product.attributes.colors">
              رنگ: {{ item.product.attributes.colors[0].name }}
            </li>
            <li class="nav-item" v-if="item.product.attributes.size">
              اندازه: {{ item.product.attributes.size }}
            </li>
          </ul>
        </b-col>
        ...
        <b-col sm="6" md="4" xl="3" class="text-md-center">
          <h5 class="mb-0" v-if="item.product.sale && item.product.price">
            <span v-if="lang==='en'">{{ currency }}{{ discountedPrice(item.product.price, item.product.sale.discount, item.product.sale.type) }}</span>
            <span v-else-if="lang==='fa'">{{ discountedPrice(item.product.price, item.product.sale.discount, item.product.sale.type) }} {{ currency }}</span>
          </h5>
          <h5 class="mb-0" v-else>
            <span v-if="lang==='en'">{{ currency }}{{ item.product.price && item.product.price.toFixed(2) }}</span>
            <span v-else-if="lang==='fa'">{{ item.product.price && item.product.price.toFixed(2) }} {{ currency }}</span>
          </h5>
        </b-col>
